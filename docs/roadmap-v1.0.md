...
Agreed direction from the review, in order: ① patch-lineage breadcrumb (extends B5's snapshot into a ring) — **✅ shipped** ("Recent sounds", 2026-06-20), ② swing + Euclidean/arp/mutate melody tools, ③ Parallax Daily (date-seeded surprise — refactor `surprise.ts`/`randomizeMelody` to take an injected RNG when the time comes), ④ un-defer one-loop audio export (MediaRecorder tap) — **✅ shipped** (2026-06-21; realtime MediaRecorder tap on masterGain → compressed WebM/Opus, not WAV; fixed +2 s release tail; ear-check confirmed 2026-07-07), ⑤ v1.2 marquee: port **Rings** as engine #4 — **✅ shipped** (2026-08-09, tagged `v1.2.0`; 12 models incl. Disastrous Peace, Soundboard theme, headless pitch calibration ±8¢, 85 tests; brief ear test done, full eye pass outstanding — shipped consciously with rollback = revert the registry entry `179aca3`)., ⑥ 6-op **FM** as engine #5 plus the **flowsheet teacher** — **✅ built and gate-passed, untagged** (phases 0–9 complete 2026-09-12: msfa port at 44.1 kHz, 14 hand-authored voices, the `graph` theme, per-operator scope taps, and the flowsheet view at `#view=flowsheet`; 222 tests). Human gate closed 2026-09-12 — ear pass on all fourteen voices done, `graph` theme eye pass done (it produced `f3eb1e2`, the gridline fix). The Firefox/Safari worklet pass is **deferred to far-deferred work** and gates nothing; the tap cost figure is Chromium-only by choice. Unlike Rings this carries no version tag yet — that is a separate call. Spec: `docs/superpowers/specs/2026-09-10-fm-engine-and-flowsheet.md`. Details: `reviews/2026-06-11-next-level-ideas.md`. The do-NOT-build list there is policy.
