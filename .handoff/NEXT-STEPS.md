...
- [ ] **Increment 2 — detection.** `audio/fft.ts` + `audio/sample-analysis.ts` (pitch = autocorrelation, brightness = FFT centroid, envelope = RMS) → wire the "Detected" readouts (auto-run on region change). Unit-check vs synthesized buffers (sine@440→A4; saw centroid > sine).
- [ ] **Increment 3 — suggestion + Apply.** `audio/suggest.ts` ranks models across all engines by `family` + `listenFor/goodFor` vs detected features → top suggestion + one-click **Apply starting patch** (`startEngine` if engine differs → `patchStore.set`; the binding pushes model + params).
...
Polyphony · Web MIDI input · audio recording/export · insert FX. *(2nd/3rd engine no longer deferred — Plaits + Laxsynth shipped 2026-06-07.)*
